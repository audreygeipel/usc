...
void print_intro() {
		...
		cout << "(5) Command T" << endl;
		cout << "(6) Print tickets and exit" << endl;
}
...
int main() {
	...
	while(cont){
        ...
        cout << new_ticket_order << endl;
			...
			case 5:
        if(tickets.size() >= 1){
			    new_ticket_order = tickets[0];
          for (unsigned int i=1; i <= tickets.size(); i++){
            new_ticket_order += tickets[i];
          }
